...
__webpack_require__.d(__webpack_exports__, {
    setOnRefreshToken: ()=>setOnRefreshToken,
    Profile: ()=>Profile,
    getCallableNumber: ()=>getCallableNumber,
    obfuscateToken: ()=>obfuscateToken,
    IndirectTransfer: ()=>IndirectTransfer,
    SipLine: ()=>SipLine,
    getDisplayableNumber: ()=>getDisplayableNumber,
    Agent: ()=>domain_Agent,
    ...
    MeetingAuthorization: ()=>MeetingAuthorization,
    ...
    Session: ()=>Session,
    setOnRefreshTokenError: ()=>setOnRefreshTokenError,
    ...
    SessionDescriptionHandler: ()=>lib_WazoSessionDescriptionHandler,
    default: ()=>src_0,
    ...
    CallApi: ()=>CallApi,
    AdHocAPIConference: ()=>AdHocAPIConference,
    guessParsePhoneNumber: ()=>guessParsePhoneNumber,
...
const external_js_base64_namespaceObject = require("js-base64");
...
class IssueReporter_IssueReporter {
    ...
    log(level, ...args) {
        ...
        const date = external_moment_default()().format('YYYY-MM-DD HH:mm:ss.SSS');
...
class ApiRequester {
    ...
    static base64Encode(str) {
        ...
        return external_js_base64_namespaceObject.Base64.encode(str);
    }
...
class Meeting {
    ...
    static parse(plain) {
        return new Meeting({
            ...
            creationTime: external_moment_default()(plain.creation_time).toDate(),
...
class ChatMessage {
    ...
    static parse(plain) {
        return new ChatMessage({
            uuid: plain.uuid,
            date: external_moment_default()(plain.created_at).toDate(),
...
class Voicemail {
    ...
    static parse(plain) {
        return new Voicemail({
            id: plain.id,
            date: external_moment_default()(1000 * plain.timestamp).toDate(),
...
class Call {
    ...
    static parse(plain) {
        return new Call({
            ...
            startingTime: external_moment_default()(plain.creation_time).toDate(),
            answerTime: external_moment_default()(plain.answer_time).toDate(),
            hangupTime: external_moment_default()(plain.hangup_time).toDate(),
...
const session_state_namespaceObject = require("sip.js/lib/api/session-state");
...
class CallSession {
    ...
    isEstablished() {
        return this.sipStatus === session_state_namespaceObject.SessionState.Established;
    }
    isTerminating() {
        return this.sipStatus === session_state_namespaceObject.SessionState.Terminating;
    }
    isTerminated() {
        return this.sipStatus === session_state_namespaceObject.SessionState.Terminated;
    }
    isConference() {
...
class Emitter {
    eventEmitter;
    constructor(){
        this.eventEmitter = new (external_events_default())();
    }
    on(event, callback) {
...
const jsonToGraphQLQuery_namespaceObject = require("json-to-graphql-query/lib/jsonToGraphQLQuery");
...
const dird = (client, baseUrl)=>({
        ...
        findMultipleContactsByNumber: (numbers, fields)=>{
            const query = (0, jsonToGraphQLQuery_namespaceObject.jsonToGraphQLQuery)({
                me: {
                    contacts: {
                        __args: {
                            profile: 'default',
...
class Recording {
    ...
    static parse(plain) {
        return new Recording({
            ...
            end: plain.end_time ? external_moment_default()(plain.end_time).toDate() : null,
            start: external_moment_default()(plain.start_time).toDate(),
            uuid: plain.uuid
        });
    }
...
class CallLog {
    ...
    static parse(plain) {
        return new CallLog({
            answer: plain.answer ? external_moment_default()(plain.answer).toDate() : null,
            ...
            start: external_moment_default()(plain.start).toDate(),
            end: plain.end ? external_moment_default()(plain.end).toDate() : null,
...
const external_jsrsasign_namespaceObject = require("jsrsasign");
...
const { jws } = external_jsrsasign_namespaceObject.KJUR;
const swarmKey = external_jsrsasign_namespaceObject.KEYUTIL.getKey(pubkey);
...
class Session {
    ...
    static parse(plain) {
        ...
        if (token) {
            ...
            if (isValid) {
                const decodedToken = jws.JWS.readSafeJSONString((0, external_jsrsasign_namespaceObject.b64utoutf8)(token.split('.')[1]));
        ...
        return new Session({
            ...
            expiresAt: external_moment_default().utc(plain.data.utc_expires_at).toDate(),
...
class WebSocketClient extends Emitter {
    ...
    connect() {
        ...
        this.socket = new (external_reconnecting_websocket_default())(this._getUrl.bind(this), [], this.options);
...
const user_agent_state_namespaceObject = require("sip.js/lib/api/user-agent-state");
const parser_namespaceObject = require("sip.js/lib/core/messages/parser");
const methods_constants_namespaceObject = require("sip.js/lib/core/messages/methods/constants");
const uri_namespaceObject = require("sip.js/lib/grammar/uri");
const user_agent_namespaceObject = require("sip.js/lib/api/user-agent");
const modifiers_namespaceObject = require("sip.js/lib/platform/web/modifiers/modifiers");
const session_dialog_namespaceObject = require("sip.js/lib/core/dialogs/session-dialog");
const messager_namespaceObject = require("sip.js/lib/api/messager");
const registerer_state_namespaceObject = require("sip.js/lib/api/registerer-state");
const transport_state_namespaceObject = require("sip.js/lib/api/transport-state");
const peer_connection_configuration_default_namespaceObject = require("sip.js/lib/platform/web/session-description-handler/peer-connection-configuration-default");
const external_getstats_namespaceObject = require("getstats");
var external_getstats_default = /*#__PURE__*/ __webpack_require__.n(external_getstats_namespaceObject);
const lib_api_namespaceObject = require("sip.js/lib/api");
const session_description_handler_namespaceObject = require("sip.js/lib/platform/web/session-description-handler/session-description-handler");
const external_sdp_transform_namespaceObject = require("sdp-transform");
var external_sdp_transform_default = /*#__PURE__*/ __webpack_require__.n(external_sdp_transform_namespaceObject);
const parseCandidate = (candidate)=>{
    if (!candidate) return null;
    const result = external_sdp_transform_default().parse(0 === candidate.indexOf('a=') ? candidate : `a=${candidate}`);
...
const getVideoDirection = (sdp)=>{
    const parsedSdp = external_sdp_transform_default().parse(sdp);
...
const fixSdp = (sdp, candidates, forcePort = true)=>{
    ...
    let fixed = external_sdp_transform_default().write(parsedSdp);
...
const addIcesInAllBundles = (sdp)=>{
    ...
    return external_sdp_transform_default().write(parsedSdp);
};
...
class WazoSessionDescriptionHandler extends session_description_handler_namespaceObject.SessionDescriptionHandler {
    gatheredCandidates;
    eventEmitter;
    isWeb;
    session;
                ...
                trackUpdates.push(new Promise((resolve)=>{
                    ...
                    resolve(null);
                        ...
                        if (oldTrack) {
                            ...
                            session_description_handler_namespaceObject.SessionDescriptionHandler.dispatchRemoveTrackEvent(localStream, oldTrack);
                        }
                        localStream.addTrack(newTrack);
                        session_description_handler_namespaceObject.SessionDescriptionHandler.dispatchAddTrackEvent(localStream, newTrack);
                    }).catch((error)=>{
...
class WebRTCClient extends Emitter {
    ...
    createUserAgent(uaConfigOverrides) {
        ...
        const ua = new user_agent_namespaceObject.UserAgent(uaOptions);
        ua.start();
        ...
        ua.transport.onMessage = (rawMessage)=>{
            const message = parser_namespaceObject.Parser.parseMessage(rawMessage, ua.transport.logger);
            ua.onTransportMessage(rawMessage);
            this.eventEmitter.emit(MESSAGE, message);
            if (message && message.method === methods_constants_namespaceObject.C.MESSAGE) ua.userAgentCore.replyStateless(message, {
                statusCode: 200
            });
        };
        return ua;
    ...
    isConnecting() {
        return this.userAgent?.transport?.state === transport_state_namespaceObject.TransportState.Connecting;
    }
    isRegistered() {
        return Boolean(this.registerer && this.registerer.state === registerer_state_namespaceObject.RegistererState.Registered);
    }
    onConnect() {
    ...
    async register(tries = 0) {
        ...
        return this._connectIfNeeded().then(()=>{
            ...
            this.registerer = new lib_api_namespaceObject.Registerer(this.userAgent, registerOptions);
            this.connectionPromise = null;
            ...
            this.registerer.stateChange.addListener((newState)=>{
                ...
                if (newState === registerer_state_namespaceObject.RegistererState.Registered && this.registerer?.state === registerer_state_namespaceObject.RegistererState.Registered) {
                ...
                } else if (newState === registerer_state_namespaceObject.RegistererState.Unregistered) this.eventEmitter.emit(UNREGISTERED);
            });
            const options = {
                requestDelegate: {
                    onReject: (response)=>{
    ...
    _monkeyPatchRegisterer(registerer) {
        ...
        registerer.unregistered = ()=>{
            if (!registerer || registerer.state === registerer_state_namespaceObject.RegistererState.Terminated) return;
            oldUnregistered();
        };
    }
    async unregister() {
        ...
        try {
            return new Promise((resolve, reject)=>{
                ...
                const onRegisterStateChange = (state)=>{
                    if (state === registerer_state_namespaceObject.RegistererState.Unregistered) {
    ...
    call(number, enableVideo, audioOnly = false, conference = false, options = {}) {
        ...
        if (audioOnly) inviterOptions.sessionDescriptionHandlerModifiersReInvite = [
            modifiers_namespaceObject.stripVideo
        ];
        const uri = this._makeURI(number);
        let session = null;
        if (uri) session = this.userAgent ? new lib_api_namespaceObject.Inviter(this.userAgent, uri, inviterOptions) : null;
        ...
        if (audioOnly) inviteOptions.sessionDescriptionHandlerModifiers.push(modifiers_namespaceObject.stripVideo);
    ...
    async hangup(session) {
        ...
        try {
            ...
            const isInviter = session instanceof lib_api_namespaceObject.Inviter;
            const actions = {
                [session_state_namespaceObject.SessionState.Initial]: isInviter ? this._cancel.bind(this, session) : this._reject.bind(this, session),
                [session_state_namespaceObject.SessionState.Establishing]: isInviter ? this._cancel.bind(this, session) : this._reject.bind(this, session),
                [session_state_namespaceObject.SessionState.Established]: this._bye.bind(this, session)
            };
    ...
    async reject(session) {
        ...
        try {
            if (session instanceof lib_api_namespaceObject.Invitation) return this._reject(session, {
                statusCode: 603,
                reasonPhrase: 'Decline'
            });
            if (session instanceof lib_api_namespaceObject.Inviter) return this._cancel(session);
        } catch (e) {
    ...
    hold(session, isConference = false, hadVideo = false) {
        ...
        if (!this._isWeb()) options.sessionDescriptionHandlerModifiers = [
            modifiers_namespaceObject.holdModifier
        ];
    ...
    message(destination, message) {
        ...
        const messager = new messager_namespaceObject.Messager(this.userAgent, uri, message);
        messager.message();
    }
    ...
    pingServer() {
        ...
        const toURI = new uri_namespaceObject.URI('sip', '', this.config.host);
        if (fromURI) {
    ...
    getState() {
        return this.userAgent ? states[this.userAgent.state] : user_agent_state_namespaceObject.UserAgentState.Stopped;
    }
    ...
    reinvite(sipSession, newConstraints = null, conference = false, audioOnly = false, iceRestart = false) {
        ...
        if (newConstraints && newConstraints.video) {
            ...
            sipSession.sessionDescriptionHandlerModifiersReInvite = modifiers.filter((modifier)=>modifier !== modifiers_namespaceObject.stripVideo);
        }
        ...
        return sipSession.invite({
            requestDelegate: {
                onAccept: (response)=>{
                    if (sipSession instanceof lib_api_namespaceObject.Inviter && sipSession.outgoingRequestMessage.body) sipSession.outgoingRequestMessage.body.body = response.message.body;
                    else if (sipSession instanceof lib_api_namespaceObject.Invitation) sipSession.incomingInviteRequest.message.body = response.message.body;
    ...
    getSipSessionId(sipSession) {
        if (!sipSession) return '';
        if (sipSession instanceof lib_api_namespaceObject.Inviter && sipSession.outgoingRequestMessage) return sipSession.outgoingRequestMessage.callId;
        if (sipSession instanceof lib_api_namespaceObject.Invitation && sipSession.incomingInviteRequest.message) return sipSession.incomingInviteRequest.message.callId;
        if (sipSession instanceof session_dialog_namespaceObject.SessionDialog) return sipSession.dialogState.callId;
        return '';
    }
    ...
    _connectIfNeeded() {
        ...
        if (this.userAgent && this.userAgent.state !== user_agent_state_namespaceObject.UserAgentState.Stopped) this.userAgent.transitionState(user_agent_state_namespaceObject.UserAgentState.Stopped);
        if (this.userAgent.transport && this.userAgent.transport.state !== transport_state_namespaceObject.TransportState.Disconnected) this.userAgent.transport.transitionState(transport_state_namespaceObject.TransportState.Disconnected);
            ...
            sessionDescriptionHandlerFactory: (session, options = {})=>{
                ...
                const sdhOptions = {
                    ...
                    peerConnectionConfiguration: {
                        ...(0, peer_connection_configuration_default_namespaceObject.defaultPeerConnectionConfiguration)(),
    ...
    _setupSession(session) {
        ...
        session.delegate.onInvite = async (request)=>{
            ...
            if (session instanceof lib_api_namespaceObject.Invitation) session.incomingInviteRequest.message.body = request.body;
            else if (session instanceof lib_api_namespaceObject.Inviter && session.outgoingInviteRequest.message.body) session.outgoingInviteRequest.message.body.body = request.body;
    ...
    _isAudioOnly(session) {
        return Boolean(session.sessionDescriptionHandlerModifiersReInvite.find((modifier)=>modifier === modifiers_namespaceObject.stripVideo));
    }
    ...
    _startSendingStats(session) {
        ...
        if (!pc) return;
        const sessionId = this.getSipSessionId(session);
        external_getstats_default()(pc, (result)=>{
    ...
    _makeURI(target) {
        return user_agent_namespaceObject.UserAgent.makeURI(`sip:${target}@${this.config.host}`);
    }
    ...
    async _accept(session, options = {}) {
        if (session.state === session_state_namespaceObject.SessionState.Terminated || session.state === session_state_namespaceObject.SessionState.Terminating) {
        ...
        if (session.state !== session_state_namespaceObject.SessionState.Initial) {
    ...
    async _reject(session, options = {}) {
        if (session.state !== session_state_namespaceObject.SessionState.Initial && session.state !== session_state_namespaceObject.SessionState.Establishing) {
    ...
    async _bye(session, options = {}) {
        if (session.state !== session_state_namespaceObject.SessionState.Established) {
...
class WebRTCPhone extends Emitter {
    ...
    _bindEvents(sipSession) {
        ...
        if (sipSession instanceof lib_api_namespaceObject.Invitation) {
        ...
        sipSession.stateChange.addListener((newState)=>{
            switch(newState){
                case session_state_namespaceObject.SessionState.Terminating:
                    ...
                    break;
                case session_state_namespaceObject.SessionState.Terminated:
                    {
    ...
    bindClientEvents() {
        ...
        this.client.on(this.client.REGISTERED, ()=>{
            ...
            if (this.shouldSendReinvite && this.currentSipSession && this.currentSipSession.state === session_state_namespaceObject.SessionState.Established) {
                this.shouldSendReinvite = false;
    ...
    _createCallSession(sipSession, fromSession, extra = {}) {
        ...
        const callSession = new CallSession({
            ...
            answered: state === session_state_namespaceObject.SessionState.Established,
...
class Phone_Phone extends Emitter {
    ...
    constructor(){
        ...
        this.SessionState = session_state_namespaceObject.SessionState;
    }
...
class Room_Room extends Emitter {
    ...
    _mapMsid(rawSdp) {
        const sdp = external_sdp_transform_default().parse(rawSdp);
...
const external_libphonenumber_js_namespaceObject = require("libphonenumber-js");
...
const guessParsePhoneNumber = (number, defaultCountry, guessingCountries = DEFAULT_GUESSING_COUNTRIES)=>{
    ...
    for(let i = 0; i < mergedCountries.length; i++){
        ...
        parsedNumber = (0, external_libphonenumber_js_namespaceObject.parsePhoneNumber)(number, country);
...
const getDisplayableNumber = (rawNumber, country, asYouType = false, guessingCountries = DEFAULT_GUESSING_COUNTRIES)=>{
    ...
    let displayValue = '';
    if (asYouType) displayValue = new external_libphonenumber_js_namespaceObject.AsYouType(country).input(number);
    else try {
