...
class Phone extends Emitter {
  ...
  async connect(options: Record<string, any> = {}, sipLine: SipLine | null | undefined = null) {
    if (this.phone) {
      // Already connected
  ...
  connectWithCredentials(server: string, sipLine: SipLine, displayName: string, rawOptions: Record<string, any> = {}) {
    if (this.phone) {
      // Already connected
      return;
    }
  ...
  async call(extension: string, withCamera = false, rawSipLine: SipLine | null | undefined = null, audioOnly = false, conference = false) {
    if (!this.phone) {
      return;
    }
