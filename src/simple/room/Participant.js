...
class Participant extends Emitter {
  ...
  onTalking(isTalking: boolean) {
    logger(logger.INFO, `${this.name} ${isTalking ? 'is talking' : 'stopped talking'} (callId: ${this.callId})`);
  ...
  updateStatus(status: Object, broadcast: boolean = true) {
    logger(logger.INFO, `Updating ${this.name}'s status`, status);
  ...
  broadcastStatus(inboundStatus: Object = null) {
    ...
    logger(logger.INFO, `Broadcasting ${this.name}'s status (callId: ${this.callId})`, status);
