...
class Participant extends Emitter {
  ...
  onTalking(isTalking: boolean) {
    logger(logger.INFO, 'onTalking', { name: this.name, isTalking, callId: this.callId });
  ...
  updateStatus(status: Object, broadcast: boolean = true) {
    logger(logger.INFO, 'Updating status', { name: this.name, status });
  ...
  broadcastStatus(inboundStatus: Object = null) {
    ...
    logger(logger.INFO, 'Broadcasting status', { callId: this.callId, status });
