...
export default class WebRTCPhone extends Emitter implements Phone {
  ...
  async sendReinvite(callSession: ?CallSession, constraints: ?Object = null, conference = false,
    audioOnly = false, iceRestart = false) {
  ...
  async stopScreenSharing(restoreLocalStream = true, callSession?: ?CallSession) {
  ...
  changeRingDevice(id: string) {
    logger.info('WebRTC phone - changing ring device', { id });
  ...
  changeAudioVolume(volume: number) {
    logger.info('WebRTC phone - changing audio volume', { volume });
  ...
  changeRingVolume(volume: number) {
    logger.info('WebRTC phone - changing ring volume', { volume });
  ...
  changeSessionVideoInputDevice(id: ?string) {
    logger.info('WebRTC phone - change session video input device', { deviceId: id });
  ...
  hold(callSession: CallSession, withEvent = true): ?Promise<any> {
  ...
  // @Deprecated
  unhold(callSession: CallSession, withEvent = true): ?Promise<any> {
  ...
  async makeCall(number: string, line: any, cameraEnabled?: boolean,
    audioOnly = false, conference = false): Promise<?CallSession> {
