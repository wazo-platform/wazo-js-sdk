...
describe('SDP utils', () => {
  ...
  describe('Parsing candidate', () => {
    it('should parse a single candidate', () => {
      const parsed = parseCandidate(candidate) as Record<string, any>;
  ...
  describe('Fixing sdp', () => {
    it('should fix a SDP without candidate or IN ip', () => {
      ...
      expect((parsed.origin as any).address).toBe('14.72.2.1');
  ...
  describe('Fixing group bundle', () => {
    it('should set a bundle for each m section', async () => {
      ...
      expect((parsed.groups?.[0] as any).mids).toBe('0 1');
    });
  });
...
`;
      ...
      expect((fixedAudioWithoutCandidate.media[0] as any).candidates[0].type).toBe('host');
      expect((fixedAudioWithoutCandidate.media[0] as any).candidates[1].type).toBe('relay');
      expect((fixedAudioWithoutCandidate.media[1] as any).candidates[0].type).toBe('host');
      expect((fixedAudioWithoutCandidate.media[1] as any).candidates[1].type).toBe('relay');
...
`;
      ...
      expect((fixedAudioWithCandidate.media?.[0] as any).candidates[0].type).toBe('srflx');
      expect((fixedAudioWithCandidate.media?.[1] as any).candidates[0].type).toBe('host');
    });
  });
});
