...
describe('object utils', () => {
  describe('convertKeysFromCamelToUnderscore', () => {
    ...
    it('should convert camel-cased keys to underscore', () => {
      ...
      value = Math.random();
      expect(convertKeysFromCamelToUnderscore({ L123: value })).toEqual({ l123: value });
    });
    ...
    it('should handle multi-leveled object', () => {
      ...
      expect(convertKeysFromCamelToUnderscore({ LoremIpsum: { DoloreSit: { Amet: value }, regular: 'reg' } }))
        .toEqual({ lorem_ipsum: { dolore_sit: { amet: value }, regular: 'reg' } });

      expect(convertKeysFromCamelToUnderscore({ LoremIpsum: { DoloreSit: { Amet: [value] }, regular: ['something'] } }))
        .toEqual({ lorem_ipsum: { dolore_sit: { amet: [value] }, regular: ['something'] } });
    });
  });
});
